...
import com.mp.ai_core.MtmdLib
import com.mp.ai_core.NativeLib
import com.mp.ai_core.StateLib
...
class ModelSwapper(private val scope: CoroutineScope) {
    ...
    suspend fun loadGeneration(
        ...
        lock.withLock {
            ...
            val ok = genLib.init(path, threads, ctxSize, temp, topK, topP, minP, mirostat, mirostatTau, mirostatEta, seed)
            if (ok) {
                currentGenPath = path
    ...
    suspend fun loadMultimodal(mmprojPath: String, threads: Int = 4): Boolean =
    ...
    suspend fun generateWithImage(
        ...
        Log.d(
            TAG,
            "generateWithImage: prompt=$prompt, imageSize=${imageWidth}x$imageHeight, mtmdReady=${mtmdLib.isReady()}, isGenerating=${_isGenerating.get()}"
        )
    ...
    suspend fun loadStateData(state: ByteArray): Boolean =
        withContext(Dispatchers.IO) { stateLib.loadStateData(state) }

    suspend fun saveStateFile(path: String): Boolean =
        withContext(Dispatchers.IO) { stateLib.saveSession(path) }

    suspend fun loadStateFile(path: String): Boolean =
        withContext(Dispatchers.IO) { stateLib.loadSession(path) }
